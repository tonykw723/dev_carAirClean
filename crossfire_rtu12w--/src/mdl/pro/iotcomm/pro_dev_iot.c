...
static void proLongLong2DevAddr(xt_s64 addr, packet_head_t *pHead)
{
	...
	MemMemcpyXTOS(pHead->devaddr, (xt_u8*)&addr + 2, DEV_ADDR_BYTES);
}
...
xt_s32 ProDevIotPack(xt_s64 addr, xt_u8 cmd, ctrl_data_in_t* di, xt_u8* buf, xt_s32* bufSize)
{
	...
	OS_ASSERT(*bufSize >= PACK_OFFSET_DATA + sizeof(packet_tail_t) + dataLen);// [68 01 02 03 04 05 06 68]共8个
	
	ptTail.checksum = proPackChkSum(&pHead, pData, dataLen);
	ptTail.etx = PACK_END_FLAG;		
	
	MemMemcpyXTOS(buf, &pHead,PACK_OFFSET_DATA);
	ret += PACK_OFFSET_DATA;
	if(dataLen > 0)
	{		
		MemMemcpyXTOS(buf + ret, pData, dataLen);
		ret += dataLen;
	}
	
	MemMemcpyXTOS(buf + ret, &ptTail, sizeof(packet_tail_t));
...
static xt_u8 proDevIotChkInBuf(pro_pkt_parse_t* p, xt_u32 *rSt, xt_u32*rLen)
{
	...
	while ((xt_s32)p->chkLen - iPktSt - p->dataLen > 0) {
		switch(p->pktSt)
		{
		...
		case PRO_PKT_ST_STX2:
			{
				iTmp = (xt_s32)p->chkLen - iPktSt - p->dataLen;
				if (iTmp >= p->needLen) {
		...
		case PRO_PKT_ST_ETX:
			{
				...
				if (iTmp >= p->needLen) {
					if (p->chkBuf[iPktSt + p->dataLen + p->needLen - 1] == PACK_END_FLAG) {
						...
						chkSum = 0;
						LOG_PRINTF(LOG_LEVEL_DEBUG, "----mdl pro: after chk: \n");
...
xt_s32 ProDevIotParse(pro_pkt_parse_t* p)
{
	...
	if (pCtrl->chkOff > 0) {
		MemMemcpyXTOS(pCtrl->chkBuf, pCtrl->chkBuf + pCtrl->chkOff,
			pCtrl->chkLen);
        pCtrl->chkOff = 0;	
		}
	...
	if (ret == SUCCESS_XT) {
		...
		if ((xt_s32)pCtrl->chkLen - stOff - stLen > 0) {
			/*MemMemcpyXTOS(pCtrl->chkBuf, pCtrl->chkBuf+stOff + stLen,
				pCtrl->chkLen - stOff - stLen);*/
		...
		pCtrl->pktLen = stLen;
		//pCtrl->chkLen = stOff + stLen;
		pCtrl->chkOff = stOff;                                                 /* 解析未完成，需保存起始标准的偏移量 */
		pCtrl->chkLen = stLen; 
