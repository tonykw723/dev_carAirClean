...
#define APP_END_ADDR	(0x08020000)
#define APP_MAX_SIZE	(0xf000)

#define APP_PAGE_SIZE	(0x100)

static uint32_t FLASHWritePage(uint32_t offset, uint8_t *pData, uint32_t len)
{
	uint32_t statrAddr = 0;
	uint32_t ret = 0;
	uint32_t off = 0;
	uint32_t data = 0;
	uint8_t c = 0;
	
	statrAddr = APPLICATION_ADDRESS;

	statrAddr += offset;
	
	FLASH_Unlock();
	
	if((statrAddr >= APP_START_ADDR) && (statrAddr < APP_END_ADDR) 
		&& (statrAddr % APP_PAGE_SIZE == 0))
	{
		...
		else
		{
			...
			while (off < APP_PAGE_SIZE)
			{
				data = *(uint32_t *)(pData + off);
...
uint32_t FLASH_If_Write(__IO uint32_t* FlashAddress, uint32_t* Data ,uint16_t DataLength)
{
  ...
  return (0);
#else
  for(i = 0; i < (DataLength * 4 / APP_PAGE_SIZE); i ++)
  {
  	if (FLASHWritePage(*FlashAddress - APPLICATION_ADDRESS, 
		(uint8_t *)Data + APP_PAGE_SIZE * i, APP_PAGE_SIZE) == 1)
		return 1;
    *FlashAddress += APP_PAGE_SIZE;
 }
  return 0;
#endif 
}
...
int32_t Ymodem_Receive (uint8_t *buf, uint8_t *pLocal)
{
            ...
            default:
              ...
              else
              {
                ...
                else
                {
				  ...
				  if (local == 0xff)
				  {
				  	if(buf[5] == 0x21 && buf[6] == 0x00)
				    {
				      local = 0;
					  *pLocal = local;
				    }
					else if(buf[5] == 0x11 && buf[6] == 0x01)
				    {
				      local = 1;
					  *pLocal = local;
					  flashdestination += APP_MAX_SIZE;
				    }
				  }
